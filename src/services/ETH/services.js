...
export async function SendTransaction(acc1) {
  const senderBalanseBefore = utils.formatEther(await provider.getBalance(acc2));
  console.log('Sender balanse before: ' + senderBalanseBefore);
  const receiverBalanseBefore = utils.formatEther(await provider.getBalance(acc1));
  ...
  if (senderBalanseBefore >= amount) {
    ...
    const senderBalanseAfter = utils.formatEther(await provider.getBalance(acc2));
    console.log('Sender balanse after: ' + senderBalanseAfter);
    const receiverBalanseAfter = utils.formatEther(await provider.getBalance(acc1));
